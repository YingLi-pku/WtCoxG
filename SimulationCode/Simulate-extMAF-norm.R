...
# sbatch -J SNPinfo --partition=bi1 --exclude=node03 --mem=20000M -t 1-0:0 --array=1-1080 -o log/%A_%a.log --wrap='Rscript SNPinfo-norm.R $SLURM_ARRAY_TASK_ID'
...
source("/gdata01/user/liying/simulation-2023-04-12/code/function0611.R")
...
params = expand.grid(prev = c(0.01,0.05,0.1)                     # disease prevalence
                     ...
                     ,theta = c( 0.01 )                          # level of batch effect
                     ,group=1:10
)
    ...
    if(!file.exists(outputfile)){
      ...
      info.SNP.causal = info.SNP.causal%>%
        ...
        mutate(batcheffect = rbinom(n(),1, bc),                                         # assign batch effect=0/1 for each SNPs
               trueMAF_ref = ifelse(batcheffect==0, trueMAF,                            # if batch effect=0 then mu.ext = mu
                                    rnorm(n(), mean = trueMAF,
                                          sd =  sqrt(theta*trueMAF*(1-trueMAF) ) )))%>% # if batch effect=1 then mu.ext ~N(mu,sigma2)
      ...
      info.SNP.all =lapply(1:(length(maf.group)-1), function(i){
        ...
        temp = lapply(n.ref , function(x){
          ...
          #var_Sbat = t(w1) %*% GRM %*% w1 + var_mu_ext
