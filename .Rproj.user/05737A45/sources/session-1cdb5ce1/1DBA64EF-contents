...
# sbatch -p C064M0256G --qos=low --ntasks-per-node=1 -J set2 -t 1-0:0 --array=1-2160 -o log/%A_%a.log --wrap='Rscript SNPinfo-trunc.R $SLURM_ARRAY_TASK_ID'
...
for(v in ((group-1)*10 + 1):(group*10)){
  ...
  outputfile = paste0("./SNPinfo_truncnorm/prev",prev,
                      "_her",h2,
                      "_bc",bc,
                      "_theta",theta,
                      "_n",n1,
  ...
  if(!file.exists(outputfile)){
    ...
    info.SNP.causal = info.SNP.causal%>%
      mutate(mu0 = apply(G.ds.causal[Phen.ds$event==0,], 2, function(x){mean(x)/2}),
             mu1 = apply(G.ds.causal[Phen.ds$event==1,], 2, function(x){mean(x)/2}),
             mu.int = (mu0 + mu1)/2,
             mu.w = prev*mu1 + (1-prev)*mu0)%>%
      mutate(batcheffect = rbinom(n(),1, bc),
             trueMAF_ref = ifelse(batcheffect==0, trueMAF,
                                  rtruncnorm(n(), a=0, b=1, mean = trueMAF, sd =  sqrt(theta*trueMAF*(1-trueMAF) ) )))%>%
      mutate(causal = "yes")
    ...
    info.SNP = info.SNP %>% mutate(mu0 = apply(G.ds.null[Phen.ds$event==0,], 2, function(x){mean(x)/2}),
